...
public class TicketServiceImpl implements TicketService {
    ...
    public TicketResponse updateTicketStatus(String ticketId, UpdateTicketStatusRequest request, String userId) {
        ...
        if (user.getRole() == Role.TECHNICIAN) {
            ...
            // Validate technician can only move from OPEN -> IN_PROGRESS or IN_PROGRESS -> RESOLVED
    ...
    public TicketResponse assignTicket(String ticketId, String assignToUserId) {
        log.info("Assigning ticket {} to user {}", ticketId, assignToUserId);

        Ticket ticket = ticketRepository.findById(ticketId)
            .orElseThrow(() -> new TicketNotFoundException("Ticket not found: " + ticketId));

        ticket.setAssignedTo(assignToUserId);
        ticket.setAssignedToName(resolveUserName(assignToUserId));
        ticket.setUpdatedAt(LocalDateTime.now());
    ...
    @Override
    public TicketResponse updateComment(String ticketId, String commentId, TicketCommentRequest request, String userId) {
